...
void Geometory::MakeCylinder()
{
	...
	{
		const float angle = twoPi * static_cast<float>(i) / static_cast<float>(circleVtx);
	...
	const int bottomCenterIdx = static_cast<int>(vtxVec.size());
	vtxVec.push_back({ {0.0f, -height / 2.0f, 0.0f}, {0.5f, 0.5f} });
	for (int i = 0; i < circleVtx; ++i)
	{
		const float angle = twoPi * static_cast<float>(i) / static_cast<float>(circleVtx);
	...
	const int bottomStart = bottomCenterIdx + 1;
	...
	const int sideTopStart = static_cast<int>(vtxVec.size());
	for (int i = 0; i < circleVtx; ++i)
	{
		const float angle = twoPi * static_cast<float>(i) / static_cast<float>(circleVtx);
		const float x = radius * std::cos(angle);
		const float z = radius * std::sin(angle);
		vtxVec.push_back({ {x, height / 2.0f, z}, {i / static_cast<float>(circleVtx), 1.0f} });
	}

	const int sideBottomStart = static_cast<int>(vtxVec.size());
	for (int i = 0; i < circleVtx; ++i)
	{
		const float angle = twoPi * static_cast<float>(i) / static_cast<float>(circleVtx);
		const float x = radius * std::cos(angle);
		const float z = radius * std::sin(angle);
		vtxVec.push_back({ {x, -height / 2.0f, z}, {i / static_cast<float>(circleVtx), 0.0f} });
	}

	for (int i = 0; i < circleVtx; ++i)
	{
		const int next = (i + 1) % circleVtx;
